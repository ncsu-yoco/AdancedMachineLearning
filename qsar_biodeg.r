...
checkSVMs <- function() {
  ...
  for (i in seq(2, 10, 2)) {              # various n_folds
...
checkNNs <- function() {
  ...
  nn = neuralnet(frmla, ds, hidden = 5, rep = 2, threshold = 0.01)
  a = compute(nn, ds[,1:41])
  b = data.frame(original = ds[,42], calc = a$net.result)
  print (dim(ds))
  print (dim(a$net.result))  
#   for (i in seq(1, 2, 2)) {              # various n_folds
#     sets <- n_cross_folds(ds, n = i)
#     #for(h in seq(1,10,3)) {
#       nn = neuralnet(frmla, sets$training, hidden = 3, rep = 3, threshold = 0.01)
...
for (i in seq(2, 10, 2)) {              # various n_folds
  sets <- n_cross_folds(dataset, n = i) 
  print (dim(sets$training))
  print (dim(sets$test))
  print (i)
  print ("**")
}